...
func (h memberHandler) GetMemberById(c *fiber.Ctx) error {
	// * Get id
	id, err := strconv.ParseUint(c.Params("member_id"), 10, 64)
	if err != nil {
		return err
	}
...
func (h memberHandler) GetMemberToken(c *fiber.Ctx) error {
	body := new(request.GetMemberTokenRequest)
	if err := c.BodyParser(body); err != nil {
		return &error_response.Error{
			Message: "Unable to parse information",
		}
	...
	result, err := h.memberService.GetMemberToken(*body)
	if err != nil {
		return err
	}
